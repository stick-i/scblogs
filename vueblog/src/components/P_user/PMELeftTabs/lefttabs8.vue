...
export default {
    ...
    methods:{
        GetData(){
            this.$axios.get("/user/like",this.config).then(res=>{
        ...
        Like(index){
            if(this.likeList[index].starStation==true){
                // 开始收藏，将blogid转化成form-data格式
                let formdata = new FormData()
				
				formdata.append("blogid", this.likeList[index].id) 
				
                this.$axios.post('/blog/like',formdata,this.config).then(res=>{
                    this.$message({
                            message: '点赞成功',
                            type: 'success'
                        });
